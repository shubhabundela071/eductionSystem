...
let token = '';
let boardId = '';
const userEmail = `shubha${uniqEmail}@mailinator.com`;
...
describe('Create User', () => {
  it('respond with json', (done) => {
    server
      ...
      .field('first_name', 'Shubha')
      .field('last_name', 'Bundela')
...
describe('Invite', () => {
  it('respond with json', (done) => {
    server
      ...
      .set('Content-Type', 'application/json')
      .send({ email: `shubhaInvited${uniqEmail}@mailinator.com`, board_id: boardId })
      .set({ Authorization: token })
      .expect(201)
      .end((err) => {
        if (err) return done(err);
